...
parameters = {'hidden_layer_sizes': [70], 'learning_rate_init': [0.05, 0.01, 0.001,0.1],'activation': ['identity', 'logistic','tanh','relu']}
...
for itr in range(20,101,20):     
    ...
    accr_test=f1_score(Y_test,predict_test)
...
for i in range(1,500,100):
    ...
    predict_test = new_dt.predict(X_test)
    # F1-Accuracy score
    accr_train=f1_score(Y_train,predict_train)
    accr_test=f1_score(Y_test,predict_test)
    train_set.append(accr_train)
    val_set.append(accr_test)
...
title = "Learning Curve wrt Epochs: ExoPlanet Identification"
plt.title(title)
plt.legend(["Training Scores","Validation Scores"])
